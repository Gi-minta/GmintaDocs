...
- **Empresa inicial** (`Persistencia/SembradorDeEmpresaInicial.cs`): when `Persistencia:AprovisionarEmpresaInicial` is true, startup also seeds a default empresa (data from `Organizacion:EmpresaInicial`, defaults "Empresa Demo"/"900000000-1") and provisions its per-tenant DB — idempotent. **Why this exists:** `login` only stamps `id_empresa` into the token (it does not verify the empresa exists), so without a provisioned empresa the admin can log in yet every business page fails with 500 `database "gmintadocs_empresa_{id}" does not exist`. The master-DB pages (Usuarios/Roles/Empresas) still work, which makes the symptom look like a per-page auth issue. (The frontend only forces a re-login on HTTP 401, not 500.)
- **Migrations** (`Persistencia/`): the master DB (Identidad + Organización) auto-migrates at startup under the same flag; per-empresa DBs are created+migrated on demand by `AprovisionadorDeEmpresa` via `POST api/empresas/{id}/aprovisionar` (or by the empresa-inicial seeder above).
